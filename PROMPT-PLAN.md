...
1. Author @SPEC.md — the product specification: purpose, game rules, deck structure (52 cards, 5 suits/categories of 10, two wild cards), card anatomy (title, meaning/essence, reflection prompt, suggested action), core mechanics (draw, daily draw, journaling, history, favorites), non-goals. If SPEC.md already exists with real content, improve and extend it rather than rewrite blindly.
...
3. Author @specs/cards/{pleasure,curiosity,beauty,connection,wonder}.md — one file per suit, 13 cards each, following the card anatomy in SPEC.md. Do NOT assume they exist — search specs/cards/ first. Never duplicate a card that already exists in another suit file; make all 52 cards unique.
