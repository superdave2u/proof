...
1. **Home** (`#/deck`) — the daily card only: the date-seeded reveal (deterministic, shown once per day). The screen is a full-height stage: the face-down card scales to the device height left over between the header and a bottom-aligned group (tap hint, title, message), so the page does not scroll. A dealt card keeps the back's 3:4 silhouette and shows the single-card layout **sealed**: the name and artwork show, everything under the artwork is censored behind tonal censor bars, and a "Reveal instructions" button over the bars opens the card detail page, where the quest, proof, and deposit flow live. Page navigation lives in the header menu. The back and sealed front are preloaded in one card stack: the container owns the idle wobble and the inner flip layer turns between faces, so a draw flips the already-loaded card in place instead of rebuilding the DOM and restarting the motion. On localhost only, a middle-left "Reveal card"/"Hide card" text control flips the daily card between face-down and revealed (gone once the card is Lived). Nothing else lives here.
